...
class OpinionsTest extends DuskTestCase
{
    ...
    public function generateRandomObjects(): array
    {
        ...
        $newOpinionsSubject = [];
        $newOpinionsSubject['opinion_id'] = $faker->randomElement(
            $opinionsRepository->all()->toArray()
        )['id'];
        $newOpinionsSubject['subject_id'] = $faker->randomElement(
            $opinionSubjectsRepository->all()->toArray()
        )['id'];
